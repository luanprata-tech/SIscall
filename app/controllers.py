...
class AuthController:
    ...
    def cadastrar_usuario(self, nome, login, senha, setor,tipo):
        ...
        sucesso = self.repo.criar(nome, login, senha_hash, tipo=tipo, setor=setor)
        if not sucesso:
    ...
    def alterar_cargo_usuario(self, user_id, novo_tipo):
        # 0 = User, 1 = Admin,2= Responsável Setor
...
class ChamadoController:
    ...
    def criar_chamado(self, usuario_id, descricao, maquina):
        ...
        if self.repo.possui_chamado_ativo(usuario_id):
            raise ValueError("Você já possui um chamado em andamento.\nAguarde a finalização dele para abrir outro.")
    ...
    def criar_chamado_com_contas(self, usuario_id, descricao, maquina, contas_selecionadas):
        ...
        if self.repo.possui_chamado_ativo(usuario_id):
            raise ValueError("Você já possui uma solicitação em andamento.\nAguarde a finalização para fazer outra.")
    ...
    def confirmar_resolucao(self, chamado_id):
        self.repo.atualizar_status(chamado_id, "Finalizado")
    ...
    def gerar_relatorio(self, data_inicio, data_fim):
        ...
        # --- CÁLCULO DOS CARDS DE STATUS ---
        stats = dados.get("stats_dict", {})
        
        # Soma total
        total = sum(stats.values())
        
        # Pega valores individuais (se não tiver, retorna 0)
        qtd_aberto = stats.get("Aberto", 0)
        qtd_andamento = stats.get("Em andamento", 0)
        qtd_finalizado = stats.get("Finalizado", 0) # + Resolvido se você usar esse status
        qtd_resolvido = stats.get("Resolvido", 0)

        qtd_fin_res = qtd_finalizado + qtd_resolvido
        
        # Função auxiliar de porcentagem
        def fmt_pct(qtd):
            if total == 0: return f"{qtd} (0%)"
            pct = (qtd / total) * 100
            return f"{qtd} ({pct:.1f}%)"

        kpis = {
            "total": str(total),
            "aberto": fmt_pct(qtd_aberto),
            "andamento": fmt_pct(qtd_andamento),
            "resolvido": fmt_pct(qtd_fin_res)
        }
        return {
            "kpis": kpis, # <--- Enviando os novos dados
            "lista_usuarios": dados["lista_usuarios"],
            "lista_setores": dados["lista_setores"]
        }