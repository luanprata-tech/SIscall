...
class UserEditDialog(QDialog, CenterMixin):
    ...
    def setup_ui(self):
        ...
        self.combo_tipo.addItems(["Usuário","Responsável Setor", "Administrador"])
        
        # 0=User, 1=Admin. Se for 1, seleciona index 1.
        if self.user.tipo == 0: self.combo_tipo.setCurrentIndex(0) #tipo user
        elif self.user.tipo == 1: self.combo_tipo.setCurrentIndex(2) #tipo admin
        else: self.combo_tipo.setCurrentIndex(1) #tipo resp_setor
    ...
    def salvar_alteracoes(self):
        try:
            ...
            if self.combo_tipo.currentIndex() == 1: novo_tipo = 2
            elif self.combo_tipo.currentIndex() == 2: novo_tipo = 1
            else: novo_tipo = 0
...
class LoginWindow(QMainWindow, CenterMixin):
    ...
    def setup_ui(self): #TELA DE LOGIN
...
class UserWindow(QMainWindow): 
    ...
    def setup_ui(self):
        ...
        if self.user.tipo == 2:  # Se for responsável de setor
            self.btn_add_user = QPushButton("Cadastrar Usuários")
            self.btn_add_user.setObjectName("MenuBtn")
            self.btn_add_user.setCheckable(True)
            self.btn_add_user.clicked.connect(lambda: self.switch_page(2))
        ...
        if self.user.tipo == 2:  # Se for responsável de setor
            self.page_register = self.create_register_user_page()
...
#================= PAGINA DE CADASTRO DE USUÁRIO (RESPONSÁVEL SETOR) =================#
    def create_register_user_page(self):
        widget = QWidget()
        layout = QVBoxLayout(widget)
        layout.setAlignment(Qt.AlignTop)
        
        header = QLabel("Cadastrar Novo Usuário do Setor")
        header.setStyleSheet("font-size: 24px; font-weight: bold; color: #2c3e50; margin: 10px 0;")
        
        form_frame = QWidget()
        form_frame.setObjectName("FormFrame")
        form_frame.setStyleSheet("""
            #FormFrame { background-color: white; border-radius: 6px; border: 1px solid #ddd; }
            QLabel { color: #333; }
        """)
        ...
        form_layout.addWidget(self.reg_nome)

        form_layout.addWidget(QLabel("Login de Acesso:"))
        self.reg_login = QLineEdit()
        form_layout.addWidget(self.reg_login)

        form_layout.addWidget(QLabel("Senha Provisória:"))
        self.reg_senha = QLineEdit()
        self.reg_senha.setEchoMode(QLineEdit.Password)
        form_layout.addWidget(self.reg_senha)

        # Info fixa do Setor (Read-only)
        lbl_setor_info = QLabel(f"Setor Vinculado: <b>{self.user.setor}</b>")
        lbl_setor_info.setStyleSheet("background-color: #e8f4f8; padding: 10px; border-radius: 4px; color: #2c3e50;")
        form_layout.addWidget(lbl_setor_info)

        btn_save = QPushButton("Cadastrar Usuário")
        btn_save.setMinimumHeight(45)
        btn_save.setCursor(Qt.PointingHandCursor)
        btn_save.clicked.connect(self.registrar_usuario_setor)
    ...
    def registrar_usuario_setor(self):
        # Verifica se o controller foi injetado pelo main.py
        if not self.auth_controller:
            QMessageBox.critical(self, "Erro Interno", "AuthController não configurado no main.py")
            return

        nome = self.reg_nome.text().strip()
        login = self.reg_login.text().strip()
        senha = self.reg_senha.text().strip()
        
        # O setor é OBRIGATORIAMENTE o mesmo do responsável logado
        setor = self.user.setor 
        ...
        try:
            # Chama o método que já criamos no AuthController
            # Note: Cargo é 0 (User Comum) e trocar_senha=True (Provisória)
            self.auth_controller.cadastrar_usuario(
                nome, login, senha, setor, tipo=0)
            
            QMessageBox.information(self, "Sucesso", f"Usuário {login} cadastrado no setor {setor}!")
            ...
            QMessageBox.warning(self, "Erro", f"Não foi possível cadastrar: {e}")
    ...
    def switch_page(self, index):
        ...
        if hasattr(self, 'btn_add_user'):#verifica se o botão existe
            self.btn_add_user.setChecked(index == 2)
    ...
    def preencher_tabela(self, table, chamados, is_admin=False):
        ...
        for i, c in enumerate(chamados):
            ...
            if c.status == "Aberto":
                status_item.setForeground(QColor("#d32f2f")) # Vermelho
            elif c.status == "Finalizado":
                status_item.setForeground(QColor("#2E7D32")) # Verde Escuro
            elif c.status == "Resolvido":
                status_item.setForeground(QColor("#1976D2")) # Azul (Aguardando usuário)
            else:
                status_item.setForeground(QColor("#F57C00")) # Laranja (Em Andamento)
            ...
            if not is_admin:
                ...
                if c.status == "Aberto":
                    # Botão EXCLUIR (apenas se ainda não foi atendido)
                    ...
                    # Aparece quando o Admin já respondeu, mas o user ainda não aceitou
                    btn_conf = QPushButton("Confirmar")
                    # Estilo verde chamativo para incentivar o clique
                    btn_conf.setStyleSheet("background-color: #27ae60; color: white; border-radius: 4px; font-weight: bold;")
                    btn_conf.setFixedSize(100, 30)
                    btn_conf.clicked.connect(lambda _, cid=c.id: self.acao_confirmar_resolucao(cid))
                    layout_btn.addWidget(btn_conf)
                    
                    # Também mostramos "Detalhes" para ele ler o que foi feito antes de confirmar
                    btn_det = QPushButton("Ler")
                    btn_det.setObjectName("Info")
                    btn_det.setFixedSize(70, 30)
                    btn_det.clicked.connect(lambda _, cid=c.id: self.ver_detalhes_chamado(cid))
                    layout_btn.addWidget(btn_det)
                    ...
                    layout_btn.addWidget(btn_details)
                
                table.setCellWidget(i, 6, cell_widget)

    def acao_confirmar_resolucao(self, chamado_id):
        """Chamado quando o usuário clica no botão verde 'Confirmar'"""
        reply = QMessageBox.question(
            self, 
            "Confirmação", 
            "O problema foi realmente resolvido?\nAo confirmar, este chamado será encerrado e você poderá abrir novos.",
            QMessageBox.Yes | QMessageBox.No
        )
        
        if reply == QMessageBox.Yes:
            try:
                self.controller.confirmar_resolucao(chamado_id)
                QMessageBox.information(self, "Sucesso", "Chamado finalizado com sucesso!")
                self.load_data() # Atualiza a tabela (O botão vai mudar para 'Detalhes')
            except Exception as e:
                QMessageBox.warning(self, "Erro", f"Erro ao confirmar: {e}")
...
class AdminWindow(QMainWindow): 
    ...
    def create_reports_page(self):
        ...
        # Frame de Filtros
        filter_frame = QFrame()
        filter_frame.setStyleSheet("background: white; border-radius: 5px; padding: 10px;")
        fl = QHBoxLayout(filter_frame)

        # CONFIGURAÇÃO DATA INICIAL (Modificado)
        self.date_inicio = QDateEdit()
        self.date_inicio.setCalendarPopup(False)  # Remove o calendário visual
        self.date_inicio.setDisplayFormat("dd/MM/yyyy") # Define formato brasileiro
        self.date_inicio.setDate(QDate.currentDate())
        # Estilo para parecer um campo de texto comum
        ...
        # CONFIGURAÇÃO DATA FINAL (Modificado)
        self.date_fim = QDateEdit()
        self.date_fim.setCalendarPopup(False) # Remove o calendário visual
        self.date_fim.setDisplayFormat("dd/MM/yyyy") # Define formato brasileiro
        ...
        btn_gerar.setFixedSize(150, 36)
        btn_gerar.clicked.connect(self.gerar_relatorios)
        btn_gerar.setStyleSheet('QPushButton { background:#2c3e50; color: white; border-radius: 4px; } QPushButton:hover { background:#34495e; }')

        # Adicionando ao layout
        fl.addWidget(QLabel("De:"))
        fl.addWidget(self.date_inicio)
        fl.addWidget(QLabel("Até:"))
        fl.addWidget(self.date_fim)
        fl.addWidget(btn_gerar)
        fl.addStretch() # Empurra tudo para a esquerda
        
        layout.addWidget(filter_frame)
        # --- NOVA LINHA DE CARDS (KPIs) ---
        # Layout horizontal para os 4 cards
        self.kpi_layout = QHBoxLayout()
        self.kpi_layout.setSpacing(15)

        # Criação dos cards com cores indicativas (borda inferior)
        self.card_kpi_total = self.create_status_card("Total", "0", "#34495e")     # Azul escuro
        self.card_kpi_aberto = self.create_status_card("Abertos", "0 (0%)", "#e74c3c") # Vermelho
        self.card_kpi_andamento = self.create_status_card("Andamento", "0 (0%)", "#f39c12") # Laranja/Amarelo
        self.card_kpi_resolvido = self.create_status_card("Resolvidos", "0 (0%)", "#27ae60") # Verde

        self.kpi_layout.addWidget(self.card_kpi_total)
        self.kpi_layout.addWidget(self.card_kpi_aberto)
        self.kpi_layout.addWidget(self.card_kpi_andamento)
        self.kpi_layout.addWidget(self.card_kpi_resolvido)
    ...
    # --- NOVAS LISTAS (TOP 5) ---
        ...
        # Cria as tabelas e guarda as referências para preencher depois
        self.box_users, self.table_users = self.create_ranking_table("Top 5 Usuários")
        self.box_sectors, self.table_sectors = self.create_ranking_table("Top 5 Setores")
        
        rank_layout.addWidget(self.box_users)
        rank_layout.addWidget(self.box_sectors)
    ...
    def create_ranking_table(self, title):
        group = QGroupBox(title)
        group.setStyleSheet("QGroupBox { font-weight: bold; color: #34495e; border: 1px solid #bdc3c7; border-radius: 5px; margin-top: 10px; } QGroupBox::title { subcontrol-origin: margin; left: 10px; padding: 0 5px; }")
        layout = QVBoxLayout(group)
        
        table = QTableWidget()
        table.setColumnCount(2)
        table.setHorizontalHeaderLabels(["Nome", "Qtd"])
        table.horizontalHeader().setSectionResizeMode(0, QHeaderView.Stretch) # Nome estica
        table.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeToContents) # Qtd ajusta
        table.verticalHeader().setVisible(False) # Remove números da linha
        table.setAlternatingRowColors(True)
        table.setEditTriggers(QAbstractItemView.NoEditTriggers) # Somente leitura
        table.setSelectionMode(QAbstractItemView.NoSelection) # Sem seleção
        table.setStyleSheet("QTableWidget { border: none; background: transparent; } QHeaderView::section { background: #ecf0f1; border: none; font-weight: bold; }")
        
        # Define altura fixa pequena para caber 5 itens sem scroll excessivo
    ...
    # Novo método auxiliar para criar card de status bonito
    def create_status_card(self, title, value, color_hex):
        frame = QFrame()
        ...
        lbl_val.setObjectName("ValueLabel") # ID para atualizar depois
        lbl_val.setStyleSheet(f"color: {color_hex}; font-size: 20px; font-weight: bold; border: none;")
    ...
    def gerar_relatorios(self):
        d_ini = self.date_inicio.date().toString("yyyy-MM-dd")
        d_fim = self.date_fim.date().toString("yyyy-MM-dd")
        try:
            data = self.controller.gerar_relatorio(d_ini, d_fim)
            
            # 1. Atualiza os Cards de KPI (Novos)
            kpis = data["kpis"]
            self.card_kpi_total.findChild(QLabel, "ValueLabel").setText(kpis["total"])
            self.card_kpi_aberto.findChild(QLabel, "ValueLabel").setText(kpis["aberto"])
            self.card_kpi_andamento.findChild(QLabel, "ValueLabel").setText(kpis["andamento"])
            self.card_kpi_resolvido.findChild(QLabel, "ValueLabel").setText(kpis["resolvido"])
            self.preencher_tabela_ranking(self.table_users, data["lista_usuarios"])
            self.preencher_tabela_ranking(self.table_sectors, data["lista_setores"])

            
        except Exception as e:
            QMessageBox.warning(self, "Erro", f"Falha ao gerar: {e}")
            traceback.print_exc()

    def preencher_tabela_ranking(self, table, dados_lista):
        table.setRowCount(0) # Limpa
        for nome, qtd in dados_lista:
            row = table.rowCount()
            table.insertRow(row)
    ...
    def create_config_page(self):
        ...
        self.combo_filter_setor.addItem("TODOS") 
        setores = ["GERH", "ASCOM", "COTRANSP", "GEATEC", "GEINFORM", "GECONF", "GEAAD", "PROJUR", "DIRAF","DITEC","AGEPLAN","PRESIDENCIA","OUVIDORIA","PROTOCOLO","AUDITORIO","LABAGUA","CONSEGER","GEMETRO","GEREMETRO","COMEL","AGEQUALI","CPI","ARRECADAÇÂO","FISCAIS","LABAGUA","LEI","LABORG","LABROMO","LABSOLOS","LABMICRO","PRE-MEDIDOS","GUARITA"]
        self.combo_filter_setor.addItems(sorted(setores))
        ...
        # --- NOVA IMPLEMENTAÇÃO: Botão de Cadastro (Canto Direito) ---
        
        # Cria um layout horizontal para os botões do rodapé
        footer_layout = QHBoxLayout()
        
        # 'addStretch' empurra o conteúdo seguinte para a direita
        footer_layout.addStretch()
        
        # Criação do botão
        self.btn_add_user = QPushButton(" + Cadastrar Usuário ")
        self.btn_add_user.setCursor(Qt.PointingHandCursor)
        # Estilo verde para destacar ação de "Criar"
        self.btn_add_user.setStyleSheet("background-color: #27ae60; color: white; padding: 10px 20px; font-weight: bold; border-radius: 4px;")
        # Conecta ao método que cria o diálogo
        self.btn_add_user.clicked.connect(self.open_cadastro_dialog)
        
        footer_layout.addWidget(self.btn_add_user)
        layout.addLayout(footer_layout)
        # -------------------------------------------------------------
        
        return widget
    
    def open_cadastro_dialog(self):
        """Abre um modal para o administrador cadastrar um novo usuário"""
        dialog = QDialog(self)
        dialog.setWindowTitle("Novo Usuário")
        dialog.setFixedSize(400, 450)
        dialog.setStyleSheet("background-color: #ffffff;") # Fundo branco limpo
        ...
        # Título do Modal
        lbl_title = QLabel("Dados do Usuário")
        lbl_title.setStyleSheet("font-size: 18px; font-weight: bold; color: #333;")
        layout.addWidget(lbl_title)
        
        # Campos de Texto
        txt_nome = QLineEdit(); txt_nome.setPlaceholderText("Nome Completo")
        txt_login = QLineEdit(); txt_login.setPlaceholderText("Login")
        txt_senha = QLineEdit(); txt_senha.setPlaceholderText("Senha")
        txt_senha.setEchoMode(QLineEdit.Password) # Oculta caracteres
        
        # Combo de Setor (Reutiliza a lista definida no create_config_page se possível, ou define aqui)
        combo_setor = QComboBox()
        combo_setor.addItem("Selecione o Setor")
        # Lista padrão usada no seu sistema
        setores = ["GERH", "ASCOM", "COTRANSP", "GEATEC", "GEINFORM", "GECONF", "GEAAD", "PROJUR", "DIRAF","DITEC","AGEPLAN","PRESIDENCIA","OUVIDORIA","PROTOCOLO","AUDITORIO","LABAGUA","CONSEGER","GEMETRO","GEREMETRO","COMEL","AGEQUALI","CPI","ARRECADAÇÂO","FISCAIS","LABAGUA","LEI","LABORG","LABROMO","LABSOLOS","LABMICRO","PRE-MEDIDOS","GUARITA"]
        combo_setor.addItems(sorted(setores))

        combo_tipo = QComboBox()
        combo_tipo.addItems(["Usuário","Responsável Setor", "Administrador"])
        ...
        btn_save.setMinimumHeight(45)
        btn_save.setCursor(Qt.PointingHandCursor)
        btn_save.setStyleSheet("background-color: #458BD2; color: white; font-weight: bold; border-radius: 4px;")
        layout.addWidget(btn_save)
        
        # Função interna para validar e salvar
        def save_action():
            nome = txt_nome.text().strip()
            login = txt_login.text().strip()
            senha = txt_senha.text().strip()
            setor = combo_setor.currentText()
            tipo = combo_tipo.currentText()
            if tipo == "Usuário": tipo = 0
            elif tipo == "Responsável Setor": tipo = 2
            else: tipo = 1

            
            # Validação simples
            if not nome or not login or not senha:
                QMessageBox.warning(dialog, "Atenção", "Preencha os campos obrigatórios.")
                return
            if setor == "Selecione o Setor":
                QMessageBox.warning(dialog, "Atenção", "Selecione um setor válido.")
                return

            try:
                # Chama o controller existente
                self.auth_controller.cadastrar_usuario(nome, login, senha, setor,tipo)
                QMessageBox.information(dialog, "Sucesso", f"Usuário {login} criado com sucesso!")
                dialog.accept() # Fecha o modal
                self.load_users() # Atualiza a tabela atrás
            except Exception as e:
                QMessageBox.critical(dialog, "Erro", f"Erro ao criar usuário: {e}")
    ...
    def load_users(self):
        ...
        for i, u in enumerate(usuarios):
            ...
            if u.tipo == 1: nome_display = u.nome + (" (Admin)")
            elif u.tipo == 2: nome_display = u.nome + (" (Resp. Setor)")
            else: nome_display = u.nome
    ...
    def preencher_tabela_admin(self, table, chamados, edit_mode):
        ...
        for i, c in enumerate(chamados):
            ...
            if edit_mode:
                ...
                elif c.status == "Em andamento" and is_mine: btn.setText("Continuar"); btn.setObjectName("SubmitBtn"); btn.setEnabled(True)
